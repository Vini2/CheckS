...
def plot_kmer_res(width, size1, size2, figformat, figdpi, kmers_in_reads, kmers_in_contigs, x_labels, output_path, prefix):
    ...
    rects1 = ax.bar(ind-width/2, kmers_in_reads, width, yerr=(0), label='Common k-mers in reads')
    rects2 = ax.bar(ind+width/2, kmers_in_contigs, width, yerr=(0), label='Common k-mers in contigs')
    ...
    plt.savefig(output_path+prefix+'kmer_res.'+figformat, format=figformat, dpi=figdpi, bbox_inches='tight')
    logger.info("Plot of k-mer analysis results can be found in "+output_path+prefix+'kmer_res.'+figformat)
...
def plot_gene_res(width, size1, size2, figformat, figdpi, genes_in_contigs, x_labels, output_path, prefix):
    ...
    fig, ax = plt.subplots(figsize=(size1,size2))
    rects1 = ax.bar(ind, genes_in_contigs, width, yerr=(0), label='Common genes in contigs')
    ...
    plt.savefig(output_path+prefix+'gene_res.'+figformat, format=figformat, dpi=figdpi, bbox_inches='tight')
    logger.info("Plot of gene analysis results can be found in "+output_path+prefix+'gene_res.'+figformat)