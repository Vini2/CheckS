...
def prep_files(ref_files, output_folders):
    ...
    for i in range(len(ref_files)):
        ...
        subprocess.run("cat "+" ".join(refs_excluding_reference)+" > "+output_folders[i]+"/refs_combined.fasta", shell=True)
        subprocess.run("cp "+ref_files[i]+" "+output_folders[i]+"/ref.fasta", shell=True)