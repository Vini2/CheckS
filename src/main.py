...
logger.info(
    "Welcome to CheckS: Checking the presence of a species in a metagenomics sample.")
...
# Run DSK
kmer_utils.run_dsk(contigs_file, contigs_file_name, reads_file_1,
                   reads_file_2, output_folders, k_val, nthreads, output_path)
...
#     Get ref k-mers
    ...
    logger.debug("Total number of k-mers of " +
                 ref_names[i]+": "+str(len(ref_kmers)))
...
#     Get k-mers common in ref and reads
    ...
    logger.debug("Unique k-mers of " +
                 ref_names[i]+" found in reads: "+str(len(common_read_kmers)))
...
#     Get k-mers common in ref and contigs
    ...
    logger.debug("Unique k-mers of " +
                 ref_names[i]+" found in contigs: "+str(len(common_contig_kmers)))
...
plot_utils.plot_kmer_res(0.3, 20, 15, 'png', 300, kmers_in_reads,
                         kmers_in_contigs, k_mer_res.keys(), output_path, prefix)
...
if not os.path.isfile(output_path+"/coverage_output.tsv"):
    subprocess.run("coverm genome --coupled "+reads_file_1+" "+reads_file_2+" --genome-fasta-files " +
                   " ".join(ref_files)+" -o "+output_path+"/coverage_output.tsv --threads "+str(nthreads), shell=True)
else:
    logger.debug("CoverM result exists.")

logger.info("CoverM results can be found in " +
            output_path+"/coverage_output.tsv")
...
if not os.path.isdir(output_path+"quast_results"):
    ...
    subprocess.run("metaquast -r "+",".join(ref_files)+" -o "+output_path +
                   "quast_results -t "+str(8)+" "+contigs_file, shell=True)

logger.info("The metaQUAST results can be found in " +
            output_path+"quast_results")
...
# Run MetaGeneMark
gene_utils.run_metagenemark(
    output_folders, contigs_file, contigs_file_name, contigs_file_ext, output_path)
...
for i in range(len(output_folders)):
    ...
    logger.debug("Total number of genes found in the reference: " +
                 str(len(ref_gene_nt_seq)))
    ...
    logger.debug("Total number of genes found in other reference: " +
                 str(len(other_gene_nt_seq)))

    # Get common genes between ref and other refs

    gene_utils.align(output_folders[i]+"refs_combined_genes.fna", output_folders[i] +
                     "ref_genes.fna", output_folders[i], "refs_mapped_genes.output", 0.9)
    ...
    gene_utils.align(output_path+"contigs_genes.fna",
                     output_folders[i]+"ref_unique_genes.fna", output_folders[i], "refs_unique_contigs_mapped_genes.output", 0.9)
    ...
    logger.debug("Unique ref genes mapped to contigs: " +
                 str(len(commons_genes)))
...
plot_utils.plot_gene_res(0.5, 20, 15, 'png', 300,
                         genes_in_contigs, gene_res.keys(), output_path, prefix)
